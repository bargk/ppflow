            ...
            for (int pt1 : pt1_bins) {
                ...
                for (int pt2 : pt2_bins) {
                    for (int ich = 0; ich < Bins::NCH; ich++) {
                        ...
                        fg[icent][itrk][pt1][pt2][ich] = (TH2D*)input->Get(fgname);
                        bg[icent][itrk][pt1][pt2][ich] = (TH2D*)input->Get(bgname);

                        Common::CheckObject2(fg[icent][itrk][pt1][pt2][ich], fgname);
                        Common::CheckObject2(bg[icent][itrk][pt1][pt2][ich], bgname);
                    }
                }
            }
        }
                ...
                for (int ich = Bins::NCH; ich < Bins::NCH + Bins::NCH_ADD; ich++) {
                    ...
                    for (int ich_add = Bins::ch_add_lo[ich - Bins::NCH]; ich_add < Bins::ch_add_up[ich - Bins::NCH]; ich_add++) {
                        if (ich_add == Bins::ch_add_lo[ich - Bins::NCH]) {
                            ...
                            fg[icent][itrk][pt1][pt2][ich] = (TH2D*)fg[icent][itrk][pt1][pt2][ich_add]->Clone(fgname);
                            bg[icent][itrk][pt1][pt2][ich] = (TH2D*)bg[icent][itrk][pt1][pt2][ich_add]->Clone(bgname);
                            fg[icent][itrk][pt1][pt2][ich]->SetTitle(fgname);
                            bg[icent][itrk][pt1][pt2][ich]->SetTitle(bgname);
                        } else {
                            // Add histograms for subsequent additional bins
                            fg[icent][itrk][pt1][pt2][ich]->Add(fg[icent][itrk][pt1][pt2][ich_add]);
                            bg[icent][itrk][pt1][pt2][ich]->Add(bg[icent][itrk][pt1][pt2][ich_add]);
                        }
                    }
                }
            }
    ...
    for (int icent : cent_bins) {
        for (int itrk : trk_bins) {
            for (int pt1 : pt1_bins) {
                ...
                for (int pt2 : pt2_bins) {
                    for (int ich : ch_bins) {
                        fg[icent][itrk][pt1][pt2][ich]->Write();
                        bg[icent][itrk][pt1][pt2][ich]->Write();
                    }
                }
            }
        }
