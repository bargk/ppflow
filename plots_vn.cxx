...
void Peripheraldep_Centdep(int ipt1,int ipt2,int ich, int ideta, std::vector<int> centbins_peripheral);
...
void Peripheraldep_Centdep(int ipt1,int ipt2,int ich, int ideta, std::vector<int>centbins_peripheral){
  ...
  std::vector<int> centbins_central=Bins::GetCentIndex({20,30,40,50,60,70,80,90,100});

  char name [600];
  char name1[600];
  char name2[600];
  sprintf(name1,"can_v%d%d_peripheralbindep_centdep",m_har,m_har);
  sprintf(name2,"pta%d_ptb%d_ch%d_deta%.2d",ipt1,ipt2,ich,ideta);
  ...
  for(int vnn_vn:{0,1}){
     ...
     for(auto icent2:centbins_peripheral){
       ...
       for(unsigned int icent_bin=0;icent_bin<centbins_central.size();icent_bin++){
         ...
         std::pair<float, float> vnn=Bins::GetVnn  (centbins_central[icent_bin],ipt1,ipt2,ich,ideta,m_har,icent2,InFile1);
         if(vnn.first<0) continue; //remove negative vnn values
         if(vnn_vn==1)           vnn=Bins::GetVnPtb(centbins_central[icent_bin],ipt1,ipt2,ich,ideta,m_har,icent2,InFile1,InFile2);
